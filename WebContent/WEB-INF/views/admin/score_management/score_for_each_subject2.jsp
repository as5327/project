						...
						<table class="table table-bordered">
							...
							<tbody>
								<c:forEach var="l" items="${list}">
									<tr>
										<td>${l.open_subject_id}</td>
										<td>${l.subject_name}</td>
										<td>${l.subject_start_date}</td>
										<td>${l.subject_end_date}</td>
										<td>${l.book_name}
											<button type="button"
												class="btn btn-outline-info btn-sm btnBook"
												style="float: right;" data-toggle="modal"
												data-target="#bookModal" value="${l.isbn_number}">교재 보기</button>
										</td>
										<td>${l.instructor_name}
											<button type="button"
												class="btn btn-outline-info btn-sm btnTreacher"
												style="float: right;" data-toggle="modal"
												data-target="#teacherModal" value="${l.instructor_pic}">사진 보기</button>
										</td>
										<td><a
											href="${pageContext.request.contextPath}/admin/score_management/score_for_each_subject3.do">
